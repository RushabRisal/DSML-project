...
for i, group in enumerate(commodity_groups):
    ...
    plt.savefig(f"./visualizationFig/price_trends_line/{group[0]}.png")
    plt.close()
    
    plt.figure(figsize=(15, 8))
    sns.boxplot(data=daily_trend[daily_trend['Commodity'].isin(group)], x='Commodity', y='Average')
    plt.title(f"Price Distribution for {group[0]}")
    plt.xlabel("Commodity")
    plt.ylabel("Average Price")
    plt.savefig(f"./visualizationFig/price_trends_box/{group[0]}_box_plot.png")
    plt.close()